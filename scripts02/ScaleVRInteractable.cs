...
public class ScaleVRInteractable : XRBaseInteractable
{
    ...
    {
        // 2D Axis
        Axis l_axis = ScaleVR_XRcontroller.getAxis2DState(0);
        Axis r_axis = ScaleVR_XRcontroller.getAxis2DState(1);
        if (l_axis == Axis.UP || l_axis == Axis.RIGHT || r_axis == Axis.UP || r_axis == Axis.RIGHT)
        {
            count++;
            setInfo(count.ToString());
        }
        if (l_axis == Axis.DOWN || l_axis == Axis.LEFT || r_axis == Axis.DOWN || r_axis == Axis.LEFT)
        {
            count--;
            setInfo(count.ToString());
        }

        // if no controller hover on object, then return;
        if (handidx == -1)
            return;
